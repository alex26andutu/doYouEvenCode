...
var list = [
  {
    question: '1. What is the correct JavaScript syntax to change the content of the HTML element below? <br> < p id="demo" >This is a demonstration.< /p >',
  ...
  },
  {
    ...
    answers: [{ text: "< javascript >", value: 0 }, { text: "< script >", value: 1 }, { text: "< js >", value: 0 }]
  },
  {
  ...
  {
    question: '7. Where in an HTML document is the correct place to refer to an external style sheet?',
    answers: [{ text: 'In the < head > section', value: 1 }, { text: 'In the < body > section', value: 0 }, { text: 'At the end of the document', value: 1 }]
  },
  {
    question: '8. What is the correct syntax for referring to an external script called "abc.js"?',
  ...
  if (i < list.length) {
    ...
    for (var a = 0; a < userAnswers.length; a++)
      for (var b = 0; b < userAnswers[a].length; b++) {
        ...
        else if ((list[a].answers[userAnswers[a][b]].value == 1 && b < userAnswers[a].length - 1 && list[a].answers[userAnswers[a][b + 1]].value == 0) || (list[a].answers[userAnswers[a][b]].value == 1 && b < userAnswers[a].length - 2 && list[a].answers[userAnswers[a][b + 2]].value == 0)) {
          break
        }
    ...
    resultPercentage = Math.round((score / compareList.length) * 100).toString().concat('%');
    log(resultPercentage)

    lastPage.innerHTML += '<div id="finalScore"> Congrats ! You have achieved a score of </div>' + resultPercentage;

  }
}

// saveResultBtn.addEventListener('click', onSave);
// function onSave() {
//   var result = {
//     date: myDate,
...
function generateQ(i) {
  ...
  for (var j = 0; j < list[i].answers.length; j++)
    inputForm.innerHTML += '<input class="checkBox" type = "checkbox">' + list[i].answers[j].text;
}
